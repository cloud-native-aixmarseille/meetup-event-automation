...
import type { SlackFetch } from "./slack-notification-gateway-contracts.js";
...
export class SlackNotificationGateway implements NotificationGateway {
	...
	async dispatch(
		...
		if (!response.ok) {
			const rejection = SlackNotificationGateway.httpRejection(response.status);
		...
		return SlackNotificationGateway.slackApiRejection(payload.error);
	}

	private static httpRejection(
		status: number,
	): GatewayDispatchResult | undefined {
		switch (status) {
			case 400:
			case 422:
	...
	private static slackApiRejection(error: unknown): GatewayDispatchResult {
		...
		if (
			["channel_not_found", "not_in_channel", "is_archived"].includes(error)
		) {
