...
import { GithubRepositoryDispatchMailGateway } from "./github-repository-dispatch-mail-gateway.js";
...
describe("GithubRepositoryDispatchMailGateway", () => {
	...
	it.each([
		...
		"classifies HTTP %s as a definitive rejection",
		async (status, code) => {
			// Arrange
			...
			// Act
			const actual = await gateway.dispatch(mailIntent());
	...
	it.each([
		...
		"classifies a GitHub 403 with rate-limit headers as safely retryable",
		async (failure) => {
			// Arrange
			const gateway = new GithubRepositoryDispatchMailGateway(
