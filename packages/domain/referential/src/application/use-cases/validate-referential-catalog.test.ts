...
describe("ValidateReferentialCatalog", () => {
	...
	it("retains the first host contact location and each speaker location", async () => {
		// Arrange
		const input = rawCatalog({
			hosts: [
				hostRecord({ source: { path: "catalog/venues.csv", line: 5 } }),
				hostRecord({
					contactId: "contact-0002",
					source: { path: "catalog/venues.csv", line: 8 },
				}),
			],
			speakers: [
				speakerRecord({ source: { path: "catalog/presenters.csv", line: 4 } }),
			],
		});
		const validator = new ValidateReferentialCatalog();
		...
		expect(result).toMatchObject({
			isValid: true,
			catalog: {
				hosts: [{ source: { path: "catalog/venues.csv", line: 5 } }],
				speakers: [{ source: { path: "catalog/presenters.csv", line: 4 } }],
			},
		});
	});
	...
	it("rejects duplicate display names", async () => {
		...
		const actual1 = result.diagnostics.every(
			(item) => item.severity === "error",
		);
		const actual2 = result.diagnostics.every((item) => Object.isFrozen(item));
		const actual3 = Object.isFrozen(result.diagnostics);
	...
	it.each([
		...
		const actual = result.diagnostics.map((item) => item.code);
