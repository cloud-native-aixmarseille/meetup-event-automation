...
export class ReferentialIdentifiers {
	static asHostId(value: string): HostId | undefined {
		return HOST_ID_PATTERN.test(value) ? (value as HostId) : undefined;
	}

	static asContactId(value: string): ContactId | undefined {
		return CONTACT_ID_PATTERN.test(value) ? (value as ContactId) : undefined;
	}

	static asSpeakerId(value: string): SpeakerId | undefined {
